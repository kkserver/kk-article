...
func (S *ArticleService) HandleArticleQueryTask(a IArticleApp, task *ArticleQueryTask) error {
	...
	rows, err := db.Query(fmt.Sprintf("SELECT id,alias,uid,name,author,origin,title,summary,image,images,type,intime,mtime,ctime FROM %s%s %s", a.GetPrefix(), a.GetArticleTable().Name, sql.String()), args...)
